...
Homebakers는 이메일 회원가입·로그인, Google 로그인·계정 연결, 레시피 등록·수정·삭제, 표지·단계 사진 업로드, 재료·시간별 탐색, 재료 분량 조절, 조리 모드·타이머, 장보기 목록, 스크랩, 레시피·게시글 좋아요, 완성 사진 후기, 작성자 팔로우와 새 레시피 알림을 지원합니다. 계정과 게시물·장보기 목록·알림은 DB에 저장되며, 작성자만 자신의 글과 댓글·후기를 변경할 수 있습니다. 브라우저에는 작성 초안과 이전 시안 데이터만 보존합니다. 이전 시안 데이터는 내 계정에서 가져올 수 있습니다.
